          ...
          <div key={elder._id} style={s.card}>
            {editingId === elder._id ? (
              ...
              <>
                ...
                <div style={s.actions}><button style={s.primary} onClick={() => startEdit(elder)}>Edit details</button><button style={s.secondary} onClick={() => toggleActive(elder)} disabled={savingId === elder._id}>{elder.isActive ? 'Deactivate' : 'Reactivate'}</button></div>
              </>
            )}
          </div>
        ))}
